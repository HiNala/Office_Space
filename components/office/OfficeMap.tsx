  ...
  return (
    <div className="relative" style={{ width: 64, height: 48 }}>
      <div style={{ position: 'absolute', top: 0, left: 8, width: 32, height: 22, background: '#1a1a3a', border: `2px solid ${color}`, boxShadow: `0 0 6px ${color}44` }}>
        <div style={{ margin: 2, background: '#0a0a2a', height: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: 2, left: 2, width: 4, height: 2, background: color, animation: 'blink-cursor 1s infinite' }} />
          <div style={{ position: 'absolute', top: 6, left: 2, width: 8, height: 1, background: color, opacity: 0.4 }} />
...
function WaterCooler() {
  return (
    <div style={{ width: 20, height: 36, position: 'relative' }}>
      <div style={{ position: 'absolute', top: 0, left: 3, width: 14, height: 14, background: '#4a8fd4', border: '2px solid #2a5a9a', borderRadius: '50% 50% 0 0', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', bottom: -5, left: 2, width: 2, height: 2, background: '#88ccff', animation: 'float-up 3s ease-in infinite' }} />
        <div style={{ position: 'absolute', bottom: -5, right: 3, width: 2, height: 2, background: '#88ccff', animation: 'float-up 2.5s ease-in infinite 1s' }} />
        <div style={{ position: 'absolute', bottom: -5, left: 6, width: 2, height: 2, background: '#88ccff', animation: 'float-up 4s ease-in infinite 2s' }} />
      </div>
    ...
    <div className="relative w-full h-full overflow-hidden" style={{ background: 'var(--carpet-blue)' }}>
      ...
      <div className="absolute top-0 left-0 right-0 h-16 office-wall" style={{ zIndex: 1 }}>
        ...
        <div style={{ position: 'absolute', top: 8, left: '50%', transform: 'translateX(-50%)', width: 24, height: 24, borderRadius: '50%', background: '#1a1a3a', border: '2px solid #8888aa', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ width: 2, height: 8, background: '#ffffff', transformOrigin: 'bottom center', transform: `rotate(${time.getHours() * 30 + time.getMinutes() * 0.5}deg)`, position: 'absolute', bottom: '50%' }} />
          <div style={{ width: 1, height: 9, background: '#4a8fff', transformOrigin: 'bottom center', transform: `rotate(${time.getMinutes() * 6}deg)`, position: 'absolute', bottom: '50%' }} />
          <div style={{ width: 3, height: 3, background: '#ff4444', borderRadius: '50%', position: 'absolute', zIndex: 2 }} />
        </div>
      </div>
      ...
      <div className="absolute" style={{ right: '3%', top: '18%', zIndex: 2 }}>
        <div style={{ width: 36, height: 24, background: '#ccccdd', border: '2px solid #8888aa', position: 'relative' }}>
          <div style={{ position: 'absolute', top: 4, left: 4, right: 4, height: 6, background: '#1a1a3a', border: '1px solid #4a4a8a' }} />
          <div style={{ position: 'absolute', top: 12, right: 6, width: 2, height: 2, background: '#00ff88', animation: 'blink-cursor 1.5s infinite' }} />
