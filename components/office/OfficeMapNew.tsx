...
// ─── PIX PRIMITIVE ──────────────────────────────────────────────────
function Pix({ x, y, w, h, color, style = {}, children }: {
  ...
  return (
    <div style={{
      ...
      background: color, imageRendering: 'pixelated' as const, ...style
    }}>{children}</div>
  )
}
...
function BookShelf({ x, y, w = 96, shelves = 4 }: { x: number; y: number; w?: number; shelves?: number }) {
  ...
  return (
    <div style={{ position: 'absolute', left: x, top: y, width: w, height: shelves * shelfH + 8, imageRendering: 'pixelated' as const }}>
            ...
            {bookSlice.map((bc, bi) => {
              ...
              return bx + bw < w - 4 ? (
                <div key={bi}>
                  ...
                  <Pix x={bx} y={sy + shelfH - 4 - bh} w={bw} h={1} color={'#ffffff44'} />
                </div>
              ) : null
            })}
          </div>
...
// ─── DESK ──────────────────────────────────────────────────────────
function Desk({ x, y, w = 72, agentColor = C.rexBlue }: {
  x: number; y: number; w?: number; agentColor?: string;
}) {
  return (
    <div style={{ position: 'absolute', left: x, top: y, imageRendering: 'pixelated' as const }}>
      ...
      {[0, 1, 2].map(row => [0, 1, 2, 3, 4].map(col => (
        <Pix key={`k${row}-${col}`} x={34 + col * 4} y={20 + row * 2} w={3} h={1} color="#2a2a3c" />
      )))}
      {/* Desk front face */}
...
function Chair({ x, y, color = C.chairDark, hi = C.chairMid }: {
  x: number; y: number; color?: string; hi?: string;
}) {
  return (
    <div style={{ position: 'absolute', left: x, top: y, imageRendering: 'pixelated' as const }}>
...
function Plant({ x, y, size = 'med', variant = 0 }: {
  x: number; y: number; size?: 'sm' | 'med' | 'lg'; variant?: number;
}) {
  ...
  const leafColors = [[C.plantGreen, C.plantDark, C.plantHi], ['#3aaa5a', '#1a6a3a', '#5acc7a'], ['#4a9a44', '#2a6a24', '#6acc64']]
...
// ─── PENDANT LIGHT ─────────────────────────────────────────────────
function PendantLight({ x, cordH = 28 }: { x: number; y?: number; cordH?: number }) {
  return (
    <>
      <div style={{ position: 'absolute', left: x, top: 0, imageRendering: 'pixelated' as const, zIndex: 2 }}>
...
function Whiteboard({ x, y, w = 100, h = 60 }: { x: number; y: number; w?: number; h?: number }) {
  return (
    <div style={{ position: 'absolute', left: x, top: y, width: w, height: h, imageRendering: 'pixelated' as const }}>
      ...
      <Pix x={8} y={16} w={8} h={8} color="transparent" style={{ border: '2px solid #44aa44', opacity: 0.7 }} />
      <Pix x={22} y={16} w={8} h={8} color="transparent" style={{ border: '2px solid #44aa44', opacity: 0.7 }} />
      <Pix x={30} y={20} w={8} h={2} color="#44aa44" style={{ opacity: 0.7 }} />
      <Pix x={40} y={16} w={8} h={8} color="transparent" style={{ border: '2px solid #44aa44', opacity: 0.7 }} />
...
function WaterCooler({ x, y }: { x: number; y: number }) {
  ...
  useEffect(() => {
    const t = setInterval(() => setBubbleY(by => by > 2 ? by - 1 : 8), 180)
...
// ─── PRINTER ───────────────────────────────────────────────────────
function OfficePrinter({ x, y }: { x: number; y: number }) {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      ...
      const grow = setInterval(() => {
        w = Math.min(w + 2, 28)
        setPaperW(w)
        if (w >= 28) { clearInterval(grow); setTimeout(() => setPaperW(0), 2000) }
      }, 100)
    }, 15000)
    return () => clearInterval(t)
  }, [])
  return (
    <div style={{ position: 'absolute', left: x, top: y, imageRendering: 'pixelated' as const }}>
      ...
      {paperW > 0 && <Pix x={10} y={22} w={paperW} h={3} color="#eeeeee" style={{ transition: 'width 0.1s' }} />}
...
function NorthWall({ mapW }: { mapW: number }) {
  return (
    <div style={{ position: 'absolute', left: 0, top: 0, width: mapW, height: 48, imageRendering: 'pixelated' as const }}>
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full overflow-hidden" style={{ background: C.panelBg }}>
      <div style={{
        ...
        {/* ── ZONE A FLOOR — Main work area (left 55%) ── */}
        <div style={{
          position: 'absolute', left: 0, top: 48, width: MAP_W * 0.55, height: MAP_H * 0.65,
          background: `repeating-conic-gradient(${C.floorA2} 0% 25%, ${C.floorA1} 0% 50%) 0 0 / 48px 48px`,
        }} />

        {/* ── ZONE B FLOOR — Conference + lounge (top-right 45%) ── */}
        <div style={{
          position: 'absolute', left: MAP_W * 0.55, top: 48, width: MAP_W * 0.45, height: MAP_H * 0.52,
          background: C.floorB1,
        }}>
          <div style={{
            position: 'absolute', inset: 0,
            backgroundImage: `radial-gradient(${C.floorB2} 1px, transparent 1px)`,
            backgroundSize: '8px 8px',
          }} />
        </div>

        {/* ── ZONE C FLOOR — Kitchen (bottom-right 45%) ── */}
        <div style={{
          position: 'absolute', left: MAP_W * 0.55, top: 48 + MAP_H * 0.52, width: MAP_W * 0.45, height: MAP_H * 0.48 - 48,
          background: `repeating-conic-gradient(${C.floorC2} 0% 25%, ${C.floorC1} 0% 50%) 0 0 / 32px 32px`,
        }} />

        {/* ── ZONE D FLOOR — South corridor ── */}
        <div style={{
          position: 'absolute', left: 0, top: 48 + MAP_H * 0.65, width: MAP_W * 0.55, height: MAP_H * 0.35 - 48,
          background: C.floorD1,
        }} />
        ...
        {/* ── WALL EXTRAS ── */}
        <Whiteboard x={510} y={4} w={90} h={42} />
        <WallClock x={240} y={10} />

        {/* ── PENDANT LIGHTS ── */}
        <PendantLight x={100} cordH={28} />
        <PendantLight x={250} cordH={28} />
        <PendantLight x={500} cordH={24} />
        <PendantLight x={640} cordH={26} />

        {/* Light glow pools on floor */}
        {[100, 250, 500, 640].map((lx, i) => (
          <div key={i} style={{
            position: 'absolute', left: lx - 32, top: 60,
            width: 80, height: 40,
        ...
        {/* ── ROOM DIVIDER (Zone A | Zone B boundary) ── */}
        <RoomDivider x={MAP_W * 0.55 - 15} y={48} h={MAP_H - 48} />
        ...
        {/* Rex: blueprint roll */}
        <Pix x={68} y={88} w={20} h={6} color="#4a6a9a" style={{ borderRadius: '3px' }} />
        <Pix x={68} y={88} w={20} h={1} color="#6a8aba" />
        {/* Rex: sticky notes */}
        <Pix x={14} y={84} w={8} h={8} color="#ffff88" />
        <Pix x={22} y={82} w={8} h={8} color="#88ccff" />

        {/* NOVA DESK — top right of Zone A */}
        <Desk x={210} y={80} w={74} agentColor={C.novaViolet} />
        <Chair x={214} y={126} color={C.chairDark} />
        {/* Nova: sticky wall */}
        {[
          { dx: 0, c: '#ff88cc' }, { dx: 10, c: '#88ffcc' },
          { dx: 20, c: '#ffcc88' }, { dx: 30, c: '#88ccff' },
          { dx: 5, c: '#ccff88' }, { dx: 15, c: '#ff8888' },
        ].map((n, i) => (
          <Pix key={i} x={212 + n.dx} y={60 + (i > 3 ? 10 : 0)} w={8} h={8} color={n.c} style={{ opacity: 0.85 }} />
        ))}
        {/* Nova: coffee mug */}
        <Pix x={266} y={88} w={10} h={12} color="#eeeeee" />
        <Pix x={268} y={92} w={6} h={5} color="#3a2010" style={{ borderRadius: '50%' }} />
        ...
        {isConferenceMode && (
          <div style={{
            position: 'absolute', left: 460, top: 62,
        ...
        {/* Kitchen herb plants */}
        {[0, 1, 2].map(i => (
          <Plant key={`kp${i}`} x={424 + i * 28} y={326} size="sm" variant={i} />
        ))}
        ...
        {[...Array(8)].map((_, i) => (
          <Pix key={`ep${i}`} x={302 + i * 4} y={223} w={3} h={3} color="#1a1a2a"
        ...
        {[0, 1, 2].map(i => (
          <Pix key={`ps${i}`} x={56 + i * 12} y={203} w={4} h={2} color="#333344" />
        ))}
        ...
        {/* Status bar */}
        <div style={{
          position: 'absolute', bottom: 4, left: 4, zIndex: 101,
        ...
        <div style={{
          position: 'absolute', bottom: 4, right: 4, zIndex: 101,
          display: 'flex', alignItems: 'center', gap: 6,
          background: 'rgba(0,0,0,0.7)', border: '1px solid #2a2a4a',
          padding: '4px 8px', fontFamily: 'var(--font-terminal)', fontSize: '11px', color: '#666688',
        }}>
          <span>AGENTS: 5</span>
          <span>|</span>
